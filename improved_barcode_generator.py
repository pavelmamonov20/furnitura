...
def create_excel_with_padded_barcodes(start=1, end=200, output_file='штрихкоды_с_отступами.xlsx'):
    """
    Создает Excel файл с правильно размещенными штрих-кодами с отступами
    """
    ...
    # Ширина колонок (увеличили колонку C для отступов)
    ws.column_dimensions['A'].width = 8
    ws.column_dimensions['B'].width = 15
    ws.column_dimensions['C'].width = 50  # Увеличили для отступов

    # Высота строки 30 мм = 85 пунктов
    ROW_HEIGHT = 90  # Немного увеличили для лучшего качества

    print(f"Создание Excel файла со штрих-кодами с отступами...")
    print(f"Высота строки: {ROW_HEIGHT} пунктов (30 мм)")
    print(f"Отступы: 30px слева/справа, 25px сверху/снизу")
    print("=" * 60)
    ...
    for i in range(start, end + 1):
        ...
        # Создаем и вставляем штрих-код с отступами
        try:
            print(f"  Создаю с отступами: {code}")

            barcode_img = create_barcode_with_padding(code, ROW_HEIGHT)

            # Сохраняем в байтовый поток с высоким качеством
            img_bytes = BytesIO()
            barcode_img.save(img_bytes, format='PNG', dpi=(300, 300))  # Высокое качество
            img_bytes.seek(0)
            ...
            img.width = img.height * barcode_img.width / barcode_img.height  # Ширина с учетом отступов

            # Вставляем в колонку C
    ...
    if os.path.exists(final_path):
        ...
        print(f"✓ Отступы добавлены вокруг каждого штрих-кода")
...
def create_simple_excel_test_with_padding():
    ...
    print("Создание тестового файла с отступами...")

    for i in range(1, 11):  # Только 10 для теста
        code = f"CC{i:03d}"
    ...
    # Сохраняем
    filename = 'тест_штрихкоды_с_отступами.xlsx'
    wb.save(filename)
...
if __name__ == "__main__":
    print("=" * 60)
    print("ГЕНЕРАТОР ШТРИХ-КОДОВ С ОТСТУПАМИ И УЛУЧШЕННЫМ КАЧЕСТВОМ")
    print("=" * 60)

    print("Выберите вариант:")
    print("1. Полный файл (200 штрих-кодов с отступами и улучшенным качеством)")
    print("2. Тестовый файл (10 штрих-кодов с отступами и улучшенным качеством)")
    print("3. Проверить существующий файл на дубликаты")

    choice = input("\nВаш выбор (1-3): ").strip()

    if choice == "1":
        print("\nСоздание полного файла с отступами и улучшенным качеством...")
        file_path = create_excel_with_padded_barcodes(1, 200, 'штрихкоды_с_отступами.xlsx')
        check_excel_file_for_duplicates(file_path)

    elif choice == "2":
        print("\nСоздание тестового файла с отступами и улучшенным качеством...")
        file_path = create_simple_excel_test_with_padding()
        check_excel_file_for_duplicates(file_path)

    elif choice == "3":
        filename = input("Введите имя файла для проверки: ").strip()
        check_excel_file_for_duplicates(filename)

    else:
        print("\nНеверный выбор. Создаю тестовый файл с отступами и улучшенным качеством...")
        file_path = create_simple_excel_test_with_padding()
        check_excel_file_for_duplicates(file_path)
